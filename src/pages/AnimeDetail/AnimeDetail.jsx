...
// Data
import articleDetailData from '../../assets/Data/ArticleAnime.json';

function AnimeDetail() {
    
    const { id } = useParams();
    const detailData = articleDetailData.article_anime[id];

  return (
    <section className='anime-template-container'>
        {!id ? "Loading..." :
            <div className='anime-template-wrap'>
                <div className='anime-template-wallpaper-box' style={{ background : `url(${detailData.imageUrl})` }}>
                    <h2>{detailData.title}</h2>
                    <h4>ชีวิตจริงโหดร้าย เกมแห่งความตายจึงเป็นความหวัง</h4>
                </div>
                <div className='anime-template-detail-box'>
                    <div className='anime-template-detail-poster-box'>
                        <img src={detailData.posterUrl} alt="poster-pic" loading='lazy' />
                    </div>
                    <div className='anime-template-detail-content-box'>
                        <h2>เกี่ยวกับหนัง</h2>
                ...
                <div className='anime-template-content-box'>
                ...
                <><SuggestArticle data={articleDetailData.article_anime} /></>
            </div>
        }
    </section>
  );
